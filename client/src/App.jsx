...
import { medicationsMexico } from './data/medicationsMexico.js'
import './App.css'

// México medicamentos use placeholder images defined in medicationsMéxico.js

// Saudi medicamentos - replaced with Indian medicamentos
/*
const medicamentos = {
  paracetamol: [
    {
    ...
    {
      id: 'panadol-niño-niño',
      name: 'Panadol',
      ingredient: 'Paracetamol',
      concentration: 240, // mg per 5ml (Children's 5-12 años)
      volume: 5, // ml
      form: 'Sirup',
      image: panadolImg,
      ageRestriction: 'Adecuado desde 5 años old'
    }
  ],
  ibuprofen: [
    {
...
// Use Mexico medicamentos database (Spanish)
const medicamentos = { paracetamol: medicationsMexico.paracetamol, ibuprofen: medicationsMexico.ibuprofen };
...
function App({ onChangeLanguage }) {
  ...
  const [medicamentoType, setMedicamentoType] = useState('syrup') // 'syrup' or 'suppository'
  ...
  const calculateDosage = () => {
    if (!weight || !age || !ageUnit || !selectedMedicamento) {
      return
    }
    ...
    const ageInBulans = ageUnit === 'años' ? ageNum * 12 : ageNum
    ...
    if (selectedMedicamento.ingredient === 'Paracetamol') {
      ...
      if (ageInBulans < 12) {
        setResult({ error: 'Diclofenac is suitable for niño-niño aged one year o más' })
        return
      }
      
      // Weight-based dosing for diclofenac suppositories
      if (selectedMedicamento.form === 'suppository') {
        ...
        // Check if selected medicamento matches the appropriate dosis
        if (selectedMedicamento.concentration !== appropriateDosis) {
          setResult({ error: `Please select Diclofenac suppository ${appropriateDosis}mg appropriate for your niño's weight` })
          return
        }
        ...
        setResult({
          ...
          weight: weightNum,
          dosisMg: selectedMedicamento.concentration,
          suppositories: 1,
          frequency,
          maxDailyDosiss,
          isSupositoria: true
    ...
    if (selectedMedicamento.form === 'suppository') {
      ...
      let volumeNeeded = (totalDosisMg * selectedMedicamento.volume) / selectedMedicamento.concentration
      
      // Round DOWN to nearest 0.1ml for drops, 0.5ml for syrups (safety first)
      if (selectedMedicamento.form === 'Drops') {
  ...
  const changeMedicamentoType = (type) => {
    setMedicamentoType(type)
    setSelectedMedicamento(null)
    setResult(null)
  }
  ...
  const getAppropriateSuppositoriesForAge = (ageInBulans, weightNum) => {
    ...
    return allSuppositories.filter(med => {
      ...
      // Default return false for unknown medicamento types
      return false
    })
  }

  const MedicamentoCard = ({ medicamento, category }) => {
    // Check if medicamento is Ibuprofen and age is under 6 meses
    const isIbuprofen = medicamento.ingredient === 'Ibuprofen'
    const ageInBulans = ageUnit === 'años' ? parseFloat(age) * 12 : parseFloat(age)
    const isUnder6Bulans = age && ageUnit && ageInBulans < 6
    
    // Check if medicamento is Adol Drops and age is 2 años or above
    const isAdolDrops = medicamento.id === 'adol_drops'
    const is2TahunsOrAbove = age && ageUnit === 'años' && parseFloat(age) >= 2
    
    // Check if suppository is suitable for current age/weight
    let isSupositoriaUnsuitable = false
    let unsuitabilityReason = ''
    
    if (medicamento.form === 'suppository' && age && ageUnit && weight) {
      const weightNum = parseFloat(weight)
      
      if (medicamento.ingredient === 'Paracetamol') {
        const weightRange = medicamento.weightRange
        if (weightRange) {
          const weightMatch = weightRange.match(/(\d+(?:\.\d+)?)-(\d+(?:\.\d+)?)/)
          if (weightMatch) {
      ...
      if (medicamento.ingredient === 'Diclofenac') {
        if (ageInBulans < 12) {
          isSupositoriaUnsuitable = true
          unsuitabilityReason = 'Suitable for niño-niño over 1 year old'
        } else {
          if (weightNum >= 8 && weightNum <= 16) {
            if (medicamento.concentration !== 12.5) {
              isSupositoriaUnsuitable = true
              // Show the range for THIS medicamento (25mg), not the unsuitable one
              unsuitabilityReason = 'Suitable for weight 17-25 kg'
            }
          } else if (weightNum >= 17 && weightNum <= 25) {
            if (medicamento.concentration !== 25) {
              isSupositoriaUnsuitable = true
              // Show the range for THIS medicamento (12.5mg), not the unsuitable one
            ...
            if (medicamento.concentration === 12.5) {
              unsuitabilityReason = 'Suitable for weight 8-16 kg'
            } else if (medicamento.concentration === 25) {
              unsuitabilityReason = 'Suitable for weight 17-25 kg'
            } else {
    ...
    const handleClick = () => {
      ...
      if (isSupositoriaUnsuitable) {
        alert(`⚠️ Peringatan: This suppository is not suitable for your niño\n${unsuitabilityReason}`)
        return
      }
      
      if (isIbuprofen && isUnder6Bulans) {
        alert('⚠️ Peringatan: Ibuprofen medicamentos are suitable for niño-niño 6 meses and older only')
        return
      }
      
      if (isAdolDrops && is2TahunsOrAbove) {
        alert('⚠️ Peringatan: Adol Drops are suitable for niño-niño under 2 años old only')
        return
      }
    ...
    <Card 
      className={`cursor-pointer transition-all duration-200 hover:shadow-lg ${
        selectedMedicamento?.id === medicamento.id 
      ...
      <CardContent className="p-4">
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-center gap-2">
            {medicamento.images ? (
              <div className="flex gap-1">
                {medicamento.images.map((img, index) => (
                  <img 
                    key={index}
                    src={img} 
                    alt={`${medicamento.name} ${index + 1}`}
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-600">{medicamento.ingredient}</p>
            <div className="text-sm text-gray-500">
              <span>Konsentrasi: </span>
              <span className="font-bold text-blue-600" dir="ltr">
                {medicamento.form === 'suppository' 
                  ? `${medicamento.concentration}mg`
                  : `${medicamento.concentration}mg/${medicamento.volume}ml`
                }
              </span>
              <span className="mr-2">{medicamento.form}</span>
            </div>
            {medicamento.ageRestriction && (
            ...
            {(isAdolDrops && is2TahunsOrAbove) && (
              <p className="text-xs text-red-600 font-bold mt-1 bg-red-50 px-2 py-1 rounded">
                ⚠️ Untuk niño-niño under 2 años
              </p>
            )}
        ...
        {selectedMedicamento?.id === medicamento.id && weight && age && ageUnit && (
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <div className="sticky top-0 bg-white text-gray-800 pt-12 pb-6 sm:pt-6 shadow-lg border-b-2 border-gray-100 z-40">
        <div className="max-w-4xl mx-auto px-4 relative">
          {/* Compartir Button - Fixed position on mobile to align with language toggle */}
          <button
        ...
        <div className="max-w-4xl mx-auto px-4 py-6">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-3">
              Calcular dosis yang tepat untuk niño Anda berdasarkan berat badan dan jenis medicamento
            </p>
          </div>
        </div>
      </div>
          ...
          <TabsContent value="calculator" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Masukkan umur dan berat badan niño
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      >
                        <div className="text-center">
                          <div className="text-2xl mb-1">🧒</div>
                          <div>Lebih dari 1 años</div>
                          <div className="text-xs opacity-70">(1-14 años)</div>
                        </div>
                      </Button>
                    </div>
                  </div>
                  ...
                  {ageCategory && (
                    <div className="space-y-2 animate-in slide-in-from-top-2 duration-300">
                      <label className="text-sm font-medium">
                        {ageCategory === 'bebé' ? 'Select age in meses' : 'Select age in años'}
                      </label>
                      <Select value={age} onValueChange={setAge}>
                        <SelectTrigger className="text-lg">
                          <SelectValue placeholder={ageCategory === 'bebé' ? 'Select meses' : 'Select años'} />
                        </SelectTrigger>
                        <SelectContent>
                          {ageCategory === 'bebé' ? (
                            // 1-12 meses
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <h2 className="text-xl font-semibold">Pilih bentuk medicamento:</h2>
                
                {/* Medicamento Type Selection */}
                <div className="flex gap-4 justify-center">
                  <Button
                    variant={medicamentoType === 'syrup' ? 'default' : 'outline'}
                    onClick={() => changeMedicamentoType('syrup')}
                    className="flex items-center gap-2 px-6 py-3"
                  >
                    <span>💧</span>
                    <span>Sirup</span>
                  </Button>
                  {/* Suppositories not available in México */}
                </div>
              </div>

              {medicamentoType === 'syrup' && (
                <>
                  {/* Paracetamol Section */}
                  <div>
                    <div className="flex items-center gap-2 mb-4">
                      <div className="w-4 h-4 bg-blue-500 rounded-full"></div>
                      <h3 className="text-lg font-semibold text-blue-700">Medicamento Paracetamol</h3>
                    </div>
                    <div className="grid gap-4 md:grid-cols-2">
                      {medicamentos.paracetamol.map(med => (
                        <MedicamentoCard key={med.id} medicamento={med} category="paracetamol" />
                      ))}
                    </div>
                    ...
                    <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <p className="text-sm text-blue-800 leading-relaxed">
                        If you have a Paracetamol medicamento and didn't find it in the images above, you can select the medicamento with the same concentration and you will get the same required dosis. For example, concentration 
                        {' '}
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h3 className="text-lg font-semibold text-green-700">Medicamento Ibuprofen</h3>
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                      {medicamentos.ibuprofen.map(med => (
                        <MedicamentoCard key={med.id} medicamento={med} category="ibuprofen" />
                      ))}
                    </div>
                  </div>
                </>
                  ...
                  <div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                      {suppositories.paracetamol.map(med => (
                        <MedicamentoCard key={med.id} medicamento={med} category="paracetamol_supp" />
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      {suppositories.diclofenac.map(med => (
                        <MedicamentoCard key={med.id} medicamento={med} category="diclofenac_supp" />
                      ))}
                    </div>
                  </div>
                </>
            ...
            {!selectedMedicamento && (
              <div className="flex gap-4 justify-center">
                <Button 
                  ...
                  disabled={!weight || !age || !ageUnit || !selectedMedicamento}
              ...
              <CardContent>
                {result ? (
                    ...
                    <div className="space-y-4">
                      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                        ...
                        <div className="grid gap-2 text-sm">
                          ...
                          <p><strong>Medicamento:</strong> {result.medicamento.name}</p>
                          ...
                          {(result.medicamento.ingredient === 'Ibuprofen' || result.medicamento.ingredient === 'Diclofenac') && (
                        ...
                        <Alert className="bg-orange-50 border-orange-200">
                          ...
                          <AlertDescription className="text-orange-800">
                            ...
                            <ul className="list-disc mt-2 space-y-1" style={{listStylePosition: 'inside', direction: 'ltr', paddingLeft: '1.5rem'}}>
                              <li>This calculator is for guidance only. Always consult a doctor before giving any medicamento to your niño. Do not exceed the recommended dosis and do not give medicamento for more than 3-5 days without medical consultation.</li>
                              {(() => {
                                const ageValue = convertArabicToEnglish(age)
                                const ageNum = parseFloat(ageValue)
                                const ageInBulans = ageUnit === 'años' ? ageNum * 12 : ageNum
                                return ageUnit === 'meses' && ageInBulans === 1 && (
                                  <li>Untuk bebé under one month with demam, it's best not to give demam reducers and go to the hospital for examination.</li>
                                )
                              })()}
                            </ul>
                          </AlertDescription>
                  ...
                  <p className="text-gray-500 text-center py-8">
                    Masukkan umur dan berat badan niño dan pilih medicamento untuk menghitung dosis
                  </p>
                )}
              </CardContent>
            </Card>
            ...
            <div className="grid gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Learn about different types of medicamentos, how they work, and available brand names
                  </CardDescription>
                </CardHeader>
                <CardContent>
                      ...
                      <AccordionContent className="space-y-6 pt-4">
                        <div className="bg-purple-50 p-4 rounded-lg">
                          ...
                          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            <p className="text-sm text-blue-800 leading-relaxed">
                              If you have a Paracetamol medicamento and didn't find it in the images, you can select the medicamento with the same concentration and you will get the same required dosis. For example, concentration 
                              {' '}
                              <span className="font-bold text-blue-600" dir="ltr">120mg/5ml</span>
                            </p>
                          </div>
              ...
              <Card>
                <CardHeader className="p-4 md:p-6">
                  ...
                  <CardDescription className="text-xs md:text-sm">
                    Answers to the most common questions about using demam reducers for niño-niño
                  </CardDescription>
                </CardHeader>
                      ...
                      <AccordionTrigger className="text-right hover:no-underline py-3 md:py-4">
                        <div className="flex items-center gap-2 md:gap-3">
                          ...
                          <span className="text-blue-800 font-semibold text-sm md:text-lg">
                            What is the difference between paracetamol medicamentos and (ibuprofen and diclofenac) medicamentos?
                          </span>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent>
                      ...
                      <AccordionTrigger className="text-right hover:no-underline py-3 md:py-4">
                        <div className="flex items-center gap-2 md:gap-3">
                          ...
                          <span className="text-green-800 font-semibold text-sm md:text-lg">
                            Is there an interaction between paracetamol and (ibuprofen and diclofenac) medicamentos?
                          </span>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent>
                        <div className="bg-green-50 p-3 md:p-4 rounded-lg mr-0 md:mr-11">
                          <p className="text-green-800">
                            There is no interaction between them, and they can be taken at the same time, but your doctor may recommend a specific time between them, such as every 4 hours between paracetamol and ibuprofen, so you can give medicamentos throughout the day.
                          </p>
                        </div>
                      </AccordionContent>
                    </AccordionItem>
                      ...
                      <AccordionTrigger className="text-right hover:no-underline py-3 md:py-4">
                        <div className="flex items-center gap-2 md:gap-3">
                          ...
                          <span className="text-orange-800 font-semibold text-sm md:text-lg">
                            Which medicamentos should not be combined at the same time?
                          </span>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent>
                        <div className="bg-orange-50 p-3 md:p-4 rounded-lg mr-0 md:mr-11">
                          <div className="text-orange-800 space-y-2">
                            <p>
                              <strong className="text-red-600">Do not combine medicamentos containing Paracetamol</strong> at the same time - there must be 4-6 hours between them.
                            </p>
                            <p>
                              <strong className="text-red-600">And do not combine medicamentos containing Ibuprofen (Brufen) or Diclofenac</strong> at the same time - there must be 8 hours between them.
                            </p>
                          </div>
                        </div>
                      </AccordionContent>
                      ...
                      <AccordionTrigger className="text-right hover:no-underline py-3 md:py-4">
                        <div className="flex items-center gap-2 md:gap-3">
                          ...
                          <span className="text-teal-800 font-semibold text-sm md:text-lg">
                            Should medicamentos be taken after eating or on an empty stomach?
                          </span>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent>
                        <div className="bg-teal-50 p-3 md:p-4 rounded-lg mr-0 md:mr-11">
                          <div className="text-teal-800 space-y-2">
                            <p>
                              <strong className="text-blue-600">Medicamento Paracetamol</strong> can be taken on an empty stomach.
                            </p>
                            <p>
                              <strong className="text-red-600">Ibuprofen (Brufen) medicamentos</strong> are preferably taken after food or a light meal.
                            </p>
                          </div>
                        </div>
                      </AccordionContent>
              ...
              <Alert className="bg-red-50 border-red-200">
                ...
                <AlertDescription className="text-red-800">
                  ...
                  <ul className="list-disc space-y-1 text-sm" style={{listStylePosition: 'inside', direction: 'ltr', paddingLeft: '1.5rem'}}>
                    ...
                    <li>Do not give more than one type of medicamento containing the same active ingredient</li>
                    <li>Do not use medicamento for more than 3-5 days without medical consultation</li>
                    <li>Ensure there is no allergy to the medicamento before use</li>
                    <li>Keep all medicamentos out of reach of niño-niño</li>
                  </ul>
                </AlertDescription>
              </Alert>
            </div>
      ...
      {/* Compartir Modal */}
      <ShareModalEnglish isOpen={isCompartirModalOpen} onClose={() => setIsCompartirModalOpen(false)} />
    </div>
  )
}
...
const DosageTimelineVisualizer = ({ 
  ...
  // Get unique medicamentos from dosis history
  const uniqueMedicamentos = [...new Set(dosisHistory.map(dosis => dosis.medicamentoId))]
    .map(id => dosisHistory.find(dosis => dosis.medicamentoId === id))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        {uniqueMedicamentos.map(medicamento => {
          const lastDosis = getLastDosis(medicamento.medicamentoId)
          const timeUntilNext = getTimeUntilNextDosis(lastDosis)
          const progress = getProgressPercentage(lastDosis)
          const dosissIn24h = getDosissInLast24Hours(medicamento.medicamentoId)
          const canTakeNext = timeUntilNext === 0

          return (
            <div key={medicamento.medicamentoId} className="bg-white border rounded-lg p-4">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h4 className="font-semibold text-gray-900">{medicamento.medicamentoName}</h4>
                  <p className="text-sm text-gray-600">{medicamento.ingredient}</p>
                </div>
          ...
          {dosisHistory
            ...
            .map(dosis => (
              <div key={dosis.id} className="flex items-center justify-between bg-white p-2 rounded border">
                <div>
                  <div className="font-medium text-sm">{dosis.medicamentoName}</div>
